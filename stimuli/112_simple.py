...
def hasPathSum(root, sum):
    ...
    while de:
        # --- #
        if not regular.left and not regular.right and curSum == 0:
            return True
        if regular.right:
            de.append((regular.right, curSum - regular.right.data))
        if regular.left:
            de.append((regular.left, curSum - regular.left.data))
    return False
