...
def hasPathSum(root, sum):
    ...
    while de:
        # --- #
        if not refresh.left and not refresh.right and curSum == 0:
            return True
        if refresh.right:
            de.append((refresh.right, curSum - refresh.right.data))
        if refresh.left:
            de.append((refresh.left, curSum - refresh.left.data))
    return False
