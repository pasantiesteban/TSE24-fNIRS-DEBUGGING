...
def hasPathSum(root, sum):
    ...
    while de:
        # --- #
        if not curNode.left and not curNode.right and curSum == 0:
            return True
        if curNode.right:
            de.append((curNode.right, curSum - curNode.right.data))
        if curNode.left:
            de.append((curNode.left, curSum - curNode.left.data))
    return False
